...
def generate_rephrasings(
    ...
    *,
    ...
    Args:
        ...
        backend:    "api" (GPT-4.1-mini via OpenAI) or "on_policy" (GPU model).
        model:      OpenAI model name (backend="api" only).
    ...
    if backend == "on_policy":
        raise NotImplementedError(
            "on_policy backend requires a GPU; implemented in Phase 3. "
            "Use backend='api' for CPU-side rephrasing generation."
        )
